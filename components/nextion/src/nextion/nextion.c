...
#define NEX_RESP_WAIT_TIME (pdMS_TO_TICKS(CONFIG_NEX_RESP_WAIT_TIME_MS))
    ...
    nex_err_t nextion_driver_install(uart_port_t uart_num, int baud_rate, int tx_io_num, int rx_io_num, int queue_size, QueueHandle_t *event_queue)
    {
        ...
        if (event_queue != NULL)
        {
            ...
            if (queue_handle == 0)
            {
                NEX_LOGE("could not create event queue");
        ...
        {
            NEX_LOGE("could not create operation mutex");
        ...
        {
            NEX_LOGE("could not create uart task");
    ...
    nex_err_t nextion_driver_init()
    {
        ...
        {
            NEX_LOGE("could not send 'bkcmd=3'");
    ...
    nex_err_t nextion_driver_delete()
    {
        if (!nextion_is_driver_installed())
        {
            NEX_LOGI("driver already deleted");
        ...
        for (;;)
        {
            if (xQueueReceive(uart_queue, (void *)&event, (portTickType)portMAX_DELAY))
            {
                ...
                case UART_DATA:
                {
                    ...
                    {
                        NEX_LOGE("could not write to ring buffer");
                        ...
                        if (NEX_DVC_CODE_IS_EVENT(code, message_length))
                        {
                            if (event_enabled)
                            {
                                if (nextion_parse_assemble_event(ring_buffer, message_length, &nextion_event))
                                {
                                    ...
                                    {
                                        NEX_LOGW("could not send event to queue");
                                    }
                                }
                                else
                                {
                                    NEX_LOGE("could not assemble event");
                                }
                            }
                        }
                        else
                ...
                case UART_FIFO_OVF:
                    NEX_LOGW("hw fifo overflow, might need flow control");
                ...
                case UART_BUFFER_FULL:
                    NEX_LOGW("ring buffer full, might need size increase");
                ...
                case UART_PARITY_ERR:
                    NEX_LOGW("uart parity error");
                    break;

                case UART_FRAME_ERR:
                    NEX_LOGW("uart frame error");
                    break;

                default:
                    NEX_LOGI("uart event type ignored: %d", event.type);
                    break;
                }
            }
        }
